...
rule calculate_exp:
    ...
    threads: THREADS
    log:
        f"{OUT_STEP_COUNTS}logs/rsem/{{sample}}{{lane}}_{{reads}}.log"
    params:
        rsemRef = RSEMPREPREF,
        pref = PREF,
        forward_prob = RSEM_FORWARD_PROB_VALUE,
        outPref =  f"{OUT_STEP_COUNTS}{{sample}}{{lane}}_{{reads}}",
    ...
    output:
        align =  f"{OUT_STEP_COUNTS}{{sample}}{{lane}}_{{reads}}.genes.results"
    shell:"""
        mkdir -p {params.outdir} {params.logdir}
        rsem-calculate-expression  --bam --no-bam-output -p {threads} \
        --paired-end --forward-prob {params.forward_prob} \
